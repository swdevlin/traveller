...
# XengriWordGenerator produces words in !Xeng'ri — the K'kree language — using the
...
class XengriWordGenerator
  ...
  # Includes glottal stop and click phonemes characteristic of !Xeng'ri.
  VOWELS = [
    ['a',   19],
    ["'",    8],
    ['i',    6],
  ...
  def generate
    count = @dice_roller.roll(n: 1, d: 3, note: 'Xengri syllable count')
    previous_type = nil
    syllables = []
    ...
    count.times do
      ...
      type = pick_from(table, 'Xengri syllable type')
  ...
  def consonant
    pick_from(INITIAL_CONSONANTS, 'Xengri initial consonant')
  end

  def vowel
    pick_from(VOWELS, 'Xengri vowel')
  end

  def final_consonant
    pick_from(FINAL_CONSONANTS, 'Xengri final consonant')
  end
