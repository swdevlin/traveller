...
# Determines how many Incidental/Minor/Major cargo lots (and their tonnage) are
# available for shipment from +from_system+ to +to_system+, following the Freight
# procedure (Mongoose Traveller 2e, p.239). Every roll is logged on +roller+ for a
# full audit trail.
class FreightTrafficCalculator
  include HexDistance

  LOT_TYPES           = %i[incidental minor major].freeze
  LOT_SIZE_MULTIPLIER = { incidental: 1, minor: 5, major: 10 }.freeze
  POPULATION_MAX       = 12

  attr_reader :roller, :results, :shared_modifiers, :parsec_distance

  # @param system [StarSystem]
  # @param dms [Hash<Symbol, Integer>] resolved DM values, see FreightTrafficDms.for
  # @return [Array<Hash>] population/starport/tech level/zone DM breakdown for a single
  #   world, with no role suffix on the labels — suitable for display next to a system selector.
  def self.world_modifiers(system, dms)
    ...
    tl_dm = tech_level_dm(dms, tech_level_digit(system))
    modifiers << { label: "Tech Level #{tech_level_digit(system)}", value: tl_dm } if tl_dm != 0
  ...
  class << self
    ...
    def tech_level_digit(system)
      char = system.main_world_uwp&.[](8)
      return 0 if char.nil?

      HexDigit::HEX_DIGITS.index(char) || 0
    end

    def tech_level_dm(dms, digit)
      dms[:tech_level][[digit, FreightTrafficDms::TECH_LEVEL_MAX].min]
    end
  ...
  def initialize(from_system:, to_system:, campaign:, broker_effect: 0, referee_modifier: 0, seed: nil)
  ...
  def roll_for(type)
    type_dm = case type
    when :major then @dms[:major_cargo]
    when :incidental then @dms[:incidental_cargo]
    else 0
    end

    modifiers = shared_modifiers.dup
    modifiers << { label: "Rolling for #{type.to_s.capitalize} Cargo", value: type_dm } if type_dm != 0

    total_dm = modifiers.sum { |m| m[:value] }
    qualifying_total = @roller.roll(n: 2, d: 6, dm: total_dm, note: "#{type.to_s.capitalize} cargo lot qualifying roll")
    qualifying_roll = roll_detail(qualifying_total)

    dice_code = FreightTrafficTable.dice_for(qualifying_total)

    if dice_code.nil?
      { lots: 0, lot_size_rolls: [], total_tons: 0, modifiers: modifiers, qualifying_roll: qualifying_roll, lots_roll: nil }
    else
      n, d = dice_code
      lots_total = @roller.roll(n: n, d: d, dm: 0, note: "#{type.to_s.capitalize} cargo lot count")
      lots_roll = roll_detail(lots_total)
      lot_size_rolls = Array.new(lots_total) { roll_lot_size(type) }
      total_tons = lot_size_rolls.sum { |r| r[:tons] }

      { lots: lots_total, lot_size_rolls: lot_size_rolls, total_tons: total_tons, modifiers: modifiers,
        qualifying_roll: qualifying_roll, lots_roll: lots_roll }
    end
  end

  def roll_lot_size(type)
    size_total = @roller.roll(n: 1, d: 6, dm: 0, note: "#{type.to_s.capitalize} cargo lot size")
    roll_detail(size_total).merge(tons: size_total * LOT_SIZE_MULTIPLIER[type])
  end
