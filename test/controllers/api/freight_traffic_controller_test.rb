...
class Api::FreightTrafficControllerTest < ActionDispatch::IntegrationTest
  setup do
    @campaign = campaigns(:one)
    @sector = Sector.create!(name: 'Freight Traffic API Test Sector', x: 96, y: 96, abbreviation: 'Fta')
  end
  ...
  test 'calculate is unauthorised without credentials' do
    ...
    get api_freight_traffic_url(campaign_slug: @campaign.slug, from_id: from.id, to_id: to.id), as: :json
  ...
  test 'calculate returns the full JSON shape for an authenticated referee' do
    ...
    get api_freight_traffic_url(campaign_slug: @campaign.slug, from_id: from.id, to_id: to.id,
                                 broker_effect: 2, referee_modifier: -1), as: :json
    ...
    %w[incidental minor major].each do |type|
      result = body['lot_types'][type]
      assert result.key?('lots'), type
      assert result.key?('total_tons'), type
      assert result.key?('modifiers'), type
      assert result['qualifying_roll'].key?('rolls'), type
      assert result.key?('lots_roll'), type
      assert result.key?('lot_size_rolls'), type
    end
  end
  ...
  test 'calculate returns unprocessable_entity when from and to are the same system' do
    ...
    get api_freight_traffic_url(campaign_slug: @campaign.slug, from_id: system.id, to_id: system.id), as: :json
  ...
  test 'calculate returns unprocessable_entity when a system id does not resolve' do
    ...
    get api_freight_traffic_url(campaign_slug: @campaign.slug, from_id: from.id, to_id: 0), as: :json
  ...
  test 'system is unauthorised without credentials' do
    ...
    get api_freight_traffic_system_url(campaign_slug: @campaign.slug, id: system.id), as: :json
  ...
  test 'system returns unprocessable_entity when the id does not resolve' do
    ...
    get api_freight_traffic_system_url(campaign_slug: @campaign.slug, id: 0), as: :json
