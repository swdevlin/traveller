...
class NetworkLinkTest < ActiveSupport::TestCase
  ...
  test 'valid link' do
    link = NetworkLink.new(network: @network, from_star_system: @sys_a, to_star_system: @sys_b)
    assert link.valid?
  end

  test 'normalises direction so lower id is always from' do
    higher, lower = [@sys_a, @sys_b].sort_by(&:id).reverse
    link = NetworkLink.new(network: @network, from_star_system: higher, to_star_system: lower)
    link.valid?
  ...
  test 'rejects self-link' do
    link = NetworkLink.new(network: @network, from_star_system: @sys_a, to_star_system: @sys_a)
    assert link.invalid?
  ...
  test 'rejects duplicate link' do
    ...
    dup = NetworkLink.new(network: @network, from_star_system: @sys_a, to_star_system: @sys_b)
    assert dup.invalid?
  ...
  test 'duplicate detected regardless of direction' do
    NetworkLink.create!(network: @network, from_star_system: @sys_a, to_star_system: @sys_b)
    reversed = NetworkLink.new(network: @network, from_star_system: @sys_b, to_star_system: @sys_a)
