...
class TravellerMapTest < ActiveSupport::TestCase
  def setup
    @subject = TravellerMap.new
    @systems = [
      {
        'Sector' => 'Zaru',
        'SS' => 'A',
  ...
  test 'systems_to_build_plan converts systems to YAML with type STANDARD' do
    result = @subject.send(:systems_to_build_plan, @systems)
  ...
  test 'systems_to_build_plan converts hex to subsector coordinates' do
    result = @subject.send(:build_system_definition, @systems.first)
  ...
  test 'stars added' do
    ...
    result = @subject.send(:build_system_definition, @systems.second)
